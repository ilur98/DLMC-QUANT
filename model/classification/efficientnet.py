"""
Creates a EfficientNet Model as defined in:
EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks
https://arxiv.org/abs/1905.11946
Modified from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/efficientnet.py
"""

import torch.nn as nn
import math
from .efficientnet_block import conv_bn_act
from .efficientnet_block import SameConv
from .efficientnet_block import SE
from .efficientnet_block import drop_connect
from .efficientnet_block import swish

__all__ = ['cifar_efficientnetb0', 'cifar_efficientnetb1', 'cifar_efficientnetb6', 'cifar_efficientnetb7']
__ref__ = 'https://github.com/EstherBear/small-net-cifar100/blob/905cf9e624a8f7db809231ed93dd57bfaf6f02db/models/efficientnet.py#'
base_url = 'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/'
model_urls = {
    'cifar_efficientnetb0': '',
    'cifar_efficientnetb6': '',
    'cifar_efficientnetb7': '' 
}
            ...
            if isinstance(m, SameConv):
                for n in m.modules():
                    if isinstance(n, nn.Conv2d):
                        nn.init.kaiming_normal_(n.weight, mode='fan_out')
...
def cifar_efficientnetb0(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1, 1, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb1(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1, 1.1, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb2(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1.1, 1.2, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb3(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1.2, 1.4, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb4(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1.4, 1.8, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb5(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1.6, 2.2, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb6(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(1.8, 2.6, num_classes, bn_momentum, 0.2)

def cifar_efficientnetb7(pretrained=False, num_classes=100, bn_momentum=0.90):
    return efficientnet(2.0, 3.1, num_classes, bn_momentum, 0.2)