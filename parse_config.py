...
class ConfigParser:
    def __init__(self, config, resume=None, modification=None, run_id=None):
        ...
        # Init random seed if it is not appointed in config
        if 'random_seed' not in config:
            config['random_seed'] = np.random.randint(low=0, high=0x7fffffff)

        # load config file and apply modification
        self._config = _update_config(config, modification)
        self.resume = resume
        self.save_to_disk = self.config['trainer'].get('save_to_disk', True)

        if self.save_to_disk:
            # set save_dir where trained model and log will be saved.
            save_dir = Path(self.config['trainer'].get('save_dir', 'saved/'))
            ...
            setup_logging(save_dir=self.log_dir)

        else:
            setup_logging(save_dir=None)
    ...
    def from_args(cls, args, options=''):
        ...
        if args.resume is not None:
            ...
            cfg_fname = resume.parent / 'config.yaml' if args.config is None else Path(args.config)
        else:
        ...
        if resume is None and 'resume' in config:
            resume = Path(config['resume'])

        # parse custom cli options into dictionary
        modification = {opt.target: getattr(args, _get_opt_name(opt.flags)) for opt in options}
        return config, resume, modification
